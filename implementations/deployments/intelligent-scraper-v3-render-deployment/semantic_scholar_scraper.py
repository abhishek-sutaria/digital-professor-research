...
class SemanticScholarScraper:
    ...
    async def _extract_pdf_from_paper_page(self, paper_id: str, title: str = "") -> str:
        ...
        if PLAYWRIGHT_AVAILABLE:
            try:
                ...
                async with async_playwright() as playwright:
                    ...
                    if found_links:
                        ...
                        for pdf_link in found_links[3:]:
                            if await self._validate_pdf_link(pdf_link):
                                if self.verbose:
                                    self._log(f"Found valid PDF from paper page: {pdf_link[:60]}...", "SUCCESS")
                                return pdf_link
                            elif self.verbose:
                ...
                # Log errors (only print if verbose or if it's a critical error)
                if self.verbose:
    ...
    async def _extract_paper_metadata(self, paper) -> Optional[Dict]:
        ...
        try:
            ...
            if self.verbose and open_access_pdf:
            ...
            if open_access_pdf:
                ...
                if download_link and download_link.strip():
                    ...
                    if await self._validate_pdf_link(download_link):
