    ...
    try:
        ...
        for i, url in enumerate(urls):
            ...
            if info:
                ...
                # We need just the filename part relative to the downloads mount
                # filename is 'downloads/Author/Title.mp4'
                # We want '/downloads/Author/Title.mp4'
                # But we need to handle potential path differences
                
                # Simpler approach: prepare the filename we expect
                # The filename is generated by yt-dlp, so it might be sanitized.
                # Let's rely on the requested filename from info
                requested_filename = info.get('filename') # This is the full path
