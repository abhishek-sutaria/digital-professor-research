...
import { baseConfig } from "../../hooks/use-live-api";
import {
  FunctionDeclaration,
  LiveServerToolCall,
  Modality,
...
function AltairComponent() {
  ...
  useEffect(() => {
    ...
    if (!updatedConfig.responseModalities || !updatedConfig.responseModalities.length) {
      updatedConfig.responseModalities = [Modality.AUDIO];
      changed = true;
    }

    const tools = [...(updatedConfig.tools || [])];
    ...
    const hasAltairDeclaration = tools.some(
      (tool) =>
        "functionDeclarations" in tool &&
        Array.isArray((tool as any).functionDeclarations) &&
        (tool as any).functionDeclarations.some(
          (fd: FunctionDeclaration) => fd.name === declaration.name
        )
    );

    if (!hasAltairDeclaration) {
